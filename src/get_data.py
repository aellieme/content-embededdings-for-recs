...
def load_data(subset="up0.01_ir0.01"):
    ...
    # Полная матрица эмбеддингов размером (n_items, 64), заполненная нулями
    emb_full = np.zeros((len(items), 64), dtype=np.float32)
    emb_full[emb_filtered['item_idx'].values] = np.vstack(emb_filtered['embedding'].values)
    emb_matrix = emb_full
